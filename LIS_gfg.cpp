...
void solve()
{
    ...
    for(ll i=1; i<7; i++){
    	...
    	for(ll j=i-1; j>=0; j--){
    		if(a[i] > a[j]){
    			// taking maximum element among all LIS which are smaller than ith element
    			lis[i] = max(lis[j], lis[i]);
    		}
    	}
    	// because ith element anyway form LIS of length one so add 1
    	lis[i]++;
    }
    ll res = lis[0];
    for(ll i=0; i<7; i++){
