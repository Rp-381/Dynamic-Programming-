...
ll matrix_chain_multiplication(ll a[], ll i, ll j){
	if(i+1==j){
		return 0;
	}
	ll ans = INT_MAX;
	for(ll k=i+1; k<j; k++){
		ans = min(ans, matrix_chain_multiplication(a,i,k)
			      + matrix_chain_multiplication(a,k,j) 
			      + a[i]*a[k]*a[j]);
	}
	return ans;
}
void solve()
{
    ...
    ll ans = matrix_chain_multiplication(a, 0, n-1);
    cout<<ans<<endl;
}
int32_t main()
{
